    ...
    */
    ...
    namespace protocol {
        ...
        auto inspect(Inspector& f, message& x) {
            return f(caf::meta::type_name("protomsg"), x.action, x.size, x.body);
        }
    }
            ...
            switch (state) {
                case(state::action): {
                    ...
                    if (ptr == sizeof(msg.action)) {
                        msg.action = static_cast<protocol::action_t> (act_buf);
                        //switch (msg.action) {
                ...
                case(state::body_size): {
                    ...
                    msg.size |= uint16_t(tmp) << ((ptr - sizeof(msg.action)) * sizeof(char)*8);
                    ptr++;

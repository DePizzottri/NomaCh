...
BOOST_AUTO_TEST_CASE(CAF_TCP_accept_receive_one)
{
...

    {
        ...
        self->request(io, timeout, bind_atom::value, port).receive(
            ...
            [=, &self](caf::error err) {
                BOOST_FAIL(self->system().render(err));
            }
        );
...
BOOST_AUTO_TEST_CASE(CAF_TCP_gently_disconnect)
{
...

    {
        ...
        self->receive(
            ...
            after(timeout) >> [=] {
                BOOST_FAIL("'connected' receive timeout");
            }
        );
        ...
        self->receive(
            ...
            after(timeout) >> [=] {
                BOOST_FAIL("'received' receive timeout");
            }
        );
        ...
        after(timeout) >> [=] {
            BOOST_ERROR("'read_closed' receive timeout");
        });
    }
...
BOOST_AUTO_TEST_CASE(CAF_TCP_connect_to_self)
{
...

    {
        ...
        self->receive(
            [=](received, buf_type buf, size_t length, CAF_TCP::connection connection) {
                BOOST_CHECK(buf_type(buf.begin(), buf.begin() + length) == data);
            },
            after(timeout) >> [=] {
                BOOST_FAIL("'received' receive timeout");
            }
        ...
        connh->receive(
            [=, &connh](sended, size_t length, CAF_TCP::connection connection) {
                BOOST_CHECK(length == data.size());
                //connh->send(connection, CAF_TCP::do_write::value, data);
            },
            after(timeout) >> [=] {
                BOOST_FAIL("'sended' receive timeout");
            }
        );
    }
...
BOOST_AUTO_TEST_CASE(CAF_TCP_stop_while_reading)
{
...

    {
        ...
        acch->request(io, long_timeout, bind_atom::value, port).receive(
            ...
            [=, &acch](caf::error err) {
                BOOST_FAIL(acch->system().render(err));
            }
        );
        ...
        acch->receive(
            ...
            after(long_timeout) >> [=] {
                BOOST_FAIL("'connected' receive timeout on acc size");
            }
        );
        ...
        connh->receive(
            [=, &connh](CAF_TCP::failed, CAF_TCP::do_read, int code) {
                BOOST_CHECK(code == 995);
            },
            after(long_timeout) >> [=] { //TODO: may spuriously fail
                BOOST_ERROR("'read canceled' receive timeout on conn side");
            }
        );
        ...
        connh->receive(
            ...
            after(long_timeout) >> [=] { //TODO: may spuriously fail
                BOOST_ERROR("'read closed' receive timeout on conn side");
            }
        );
        ...
        acch->receive(
            ...
            after(long_timeout) >> [=] { //TODO: may spuriously fail
                BOOST_ERROR("'read closed' receive timeout on acc side");
            }
        );
        ...
        acch->receive(
            ...
            after(long_timeout) >> [=] {
                BOOST_ERROR("'closed' receive timeout on acc side");
            }
        );
